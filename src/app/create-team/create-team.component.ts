...
export class CreateTeamComponent implements OnInit {
  constructor(private apiService:ApiService,private activatedRoute: ActivatedRoute, private router: Router,private storage:LocalStorageService, public dialog: MatDialog) {
  }
  ...
  ngOnInit(): void {
    ...
    if((window.location.pathname.indexOf(';teamA') == -1) || (window.location.pathname.indexOf(';teamB') == -1)){
      ...
      if(this.activatedRoute.snapshot.paramMap.get('teamA')) {
        try {
          this.teamAName = JSON.parse(this.apiService.decryptionAES(this.activatedRoute.snapshot.paramMap.get('teamA')));
        } catch(e) {
            alert('Wrong Input');
        }
      }
      if(this.activatedRoute.snapshot.paramMap.get('teamB')) {
        try {
          this.teamBName = JSON.parse(this.apiService.decryptionAES(this.activatedRoute.snapshot.paramMap.get('teamB')));
        } catch(e) {
            alert('Wrong Input');
        }
      }
      // checking for null parameter
      if(this.teamAName == null || this.teamAName == "" || this.teamBName == null || this.teamBName == "" ){
        alert('Something Went Wrong');
        this.router.navigate(['/fantacy-team']);
      }else{
        this.apiService.getPlayers(this.teamAName,this.teamBName).then((res:any)=>{
      ...
      if(this.maxPalyers <11){
        ...
        for(var i=0;i<this.totalTeams.length;i++){
          if(this.totalTeams[i].pid == event.currentTarget.id){
            ...
            if(!this.dreamTeam.map((x:any)=>x.pid).includes(this.totalTeams[i].pid)){
